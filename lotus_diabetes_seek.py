...
def get_answer(content) -> re.Match:
    pattern = r"^.*?</think>.*<answer>\s*(yes|no)\s*</answer>$"
...
def accuracy_reward(completions, has_diabetes, **kwargs):
    """
    ...
    try:
      ...
      for completion_contents, has_diabetes_bool in zip(contents, has_diabetes):
        if has_diabetes_bool:
            has_diabetes_answer = "yes"
        else:
            has_diabetes_answer = "no"
        ...
        prediction = match
        ratio=1
        reward = 1.0*ratio if prediction == has_diabetes_answer else 0.0
        rewards.append(reward)

      return rewards
    except Exception as e:
      print(has_diabetes, completions)
      raise e
...
def test_reward_functions():
    ...
    completions = [
        ...
        [{"content": "<think>price dropped</think><answer>  NO  </answer>"}],  # Correct format with whitespace
        [{"content": "<think>analysis</think><answer>sideways</answer>"}],  # Wrong answer
        [{"content": "just saying up"}],  # Wrong format
        [{"content": "<think>going up</think><answer> YES </answer>"}],  # Correct format with mixed case
    ]

    # Sample company data with price going up
    diabetes_info_yes = [True]*len(completions)

    # Sample company data with price going down
    diabetes_info_no = [False]*len(completions)
    ...
    accuracy_results_up = accuracy_reward(completions, diabetes_info_yes)
    ...
    accuracy_results_down = accuracy_reward(completions, diabetes_info_no)
